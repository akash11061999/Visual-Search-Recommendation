...
class FeatureExtractor(object):
    def __init__(self, path_to_deploy_file, path_to_model_file, input_layer_name="data_q", gpu_mode=True, device_id=0,
    ...
    def extract_from_img(self, img, layer):
        resized_img = cv2.resize(img, (self.height, self.width), cv2.INTER_LINEAR)
    ...
    def extract_batch(self, img_paths, layer):
        ...
        for path in img_paths:
            try:
                ...
                resized_imgs.append(cv2.resize(img, (self.height, self.width), cv2.INTER_LINEAR))
